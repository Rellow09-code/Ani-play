export class Card{
    ...
    addToTail(element:HTMLElement){
        this.tail.appendChild(element)
    }
    surrondInnerHtmlVal(element_type:string, innerHtmlVal:string){
        const element:HTMLElement = document.createElement(element_type)
        element.innerHTML = innerHtmlVal
        return element
    }
    surrondInnerHtml(element_type:string, my_element:HTMLElement){
        const element:HTMLElement = document.createElement(element_type)
        element.appendChild(my_element)
        return element
    }
    render(parent: HTMLElement | null = null) {
        // Resolve parent
        if (!this.parent) {
            if (!parent) {
        ...
        // Title
        if (this.title) {
            const title = this.surrondInnerHtmlVal(
                'h2',
                this.title
            )
        ...
        // Subtitle
        if (this.subtitle) {
            const subtitle = this.surrondInnerHtmlVal(
                'h3',
                this.subtitle
            )
        ...
        // Time
        if (this.time) {
            const time = this.surrondInnerHtmlVal(
                'span',
                this.time
            )
